...
if (isset($_POST['submit'])) {
    ...
    if (empty($disease)) {
        $diseaseErr = 'Disease information is required';
    }
    if (!preg_match("/^[a-zA-Z' -]+$/", $name) || preg_match("/^\s/", $_POST['name'])) {
        $diseaseErr = 'Invalid';
    }
    if (empty($nameErr) && empty($ageErr) && empty($genderErr) && empty($bloodgroupErr) && empty($bloodcountErr) && empty($addressErr) && empty($contactErr) && empty($diseaseErr)) {
        $query = "INSERT INTO req_history (name, age, gender, blood_group, bloodcount, address, contact, disease,user_id) VALUES ('$name', '$age', '$gender', '$bloodgroup', '$bloodcount', '$address', '$contact', '$disease','$user_id')";
...
<head>
    ...
    <link rel="stylesheet" href="reqq.css">
    <script>
...
<header>
    ...
    <div class="form-container">
        <form action="" method="post">
            ...
            <span class="success-msg"><?php echo $success; ?>
            <br><br><br>
        </form>
